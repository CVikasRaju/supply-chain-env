...
Grader for the 'easy' task:
  Single T2 supplier factory fire. Full visibility. 30 days.
  Pass threshold: score >= 0.50

Exposes:
  grade(agent_fn=None, seed=42) -> dict   ← used by openenv.yaml grader path
  EasyGrader                              ← class kept for backwards compat
"""
...
def grade(agent_fn: Optional[AgentFn] = None, seed: int = 42, **kwargs) -> dict:
    ...
    """
    Top-level grade function for the 'easy' task.
    Called by the OpenEnv validator via: graders.grader_easy:grade
    """
    # Lazy imports so the module can be imported without supply_chain_env installed
    try:
        import sys, os
        ...
        return {"task_id": "easy", "score": 0.0, "passed": False, "details": f"Import error: {e}"}
    ...
    try:
        ...
        return {
            ...
            "score": score,
            "passed": passed,
            "details": f"Easy task: {len(rewards)} steps. Score: {score:.4f}",
        }
