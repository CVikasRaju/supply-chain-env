...
Grader for the 'medium' task:
  Geopolitical block cascading T4→T3. Partial info. 60 days.
  Pass threshold: score >= 0.50

Exposes:
  grade(agent_fn=None, seed=42) -> dict   ← used by openenv.yaml grader path
  MediumGrader                            ← class kept for backwards compat
"""
...
def grade(agent_fn: Optional[AgentFn] = None, seed: int = 42) -> dict:
    """
    Top-level grade function for the 'medium' task.
    Called by the OpenEnv validator via: graders.grader_medium:grade
    """
    try:
        import sys, os
        ...
        return {"task_id": "medium", "score": 0.0, "passed": False, "details": f"Import error: {e}"}
    ...
    try:
        ...
        return {
            ...
            "score": score,
            "passed": passed,
            "details": f"Medium task: {len(rewards)} steps. Score: {score:.4f}",
        }
